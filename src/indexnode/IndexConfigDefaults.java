...
public class IndexConfigDefaults implements ConfigDefaults {
	 ...
	 * @author Gary
	 */
	public static abstract class IK {
		...
		public static final String DH_ANON_TLS = "security/allow-dh-anon";
		public static final String RESERVED_ALIASES = "security/reserved-aliases";
		public static final String USER_DATABASE = "security/users-file";
	}
	...
	public IndexConfigDefaults() {
		...
		comments.put(IK.PORT, "Insecure indexnode sockets will be bound to this port. Secure indexnode sockets will be bound to (this number+1).");
		...
		comments.put(IK.FILESYSTEM_UPDATE_POOLSIZE, "The number of simultaneous filesystem share-imports that can happen.");
		...
		defaults.put(IK.DH_ANON_TLS, Boolean.TRUE.toString());
		comments.put(IK.DH_ANON_TLS, "(recommended, until PKI is implemented) Uses anonymous Diffie-Hellman key exchange between the client and this indexnode. If disabled, clients will need this indexnode's certificate, or this indexnode will need a publicly signed certificate.");
		
		defaults.put(IK.USER_DATABASE, "users.xml");
		comments.put(IK.USER_DATABASE, "The file that users are stored in. This file is almost ignored unless secure mode is on. LocalOnly users have their names reserved and may never be used by anybody, even in insecure mode.");
	}
