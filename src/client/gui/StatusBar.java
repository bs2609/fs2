...
public class StatusBar extends JPanel implements ActionListener, MouseListener {
	...
	public StatusBar(MainFrame frame) {
		...
		infoText = new JLabel("Welcome to FS2...", frame.gui.util.getImage("info"), JLabel.LEFT);
		...
		chatInfo = new JLabel("0", frame.gui.util.getImage("chat"), JLabel.LEFT);
	...
	public void setInfoTextWithIcon(String info, Icon icon) {
		infoText.setText(info);
		infoText.setIcon(icon);
	}
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == updater) {
			// 1) Speed labels:
			String upStr = Util.niceSize((long) frame.gui.ssvr.getUploadTracker().getSpeed(), true) + "/s [" + Util.niceSize(frame.gui.ssvr.getUploadSpeed(), true) + "/s]";
			upInfo.setText(upStr);
			String downStr = Util.niceSize((long) frame.gui.dc.getDownloadTracker().getSpeed(), true) + "/s [" + Util.niceSize(frame.gui.dc.getDownloadSpeed(), true) + "/s]";
